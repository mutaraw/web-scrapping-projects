...
def extract_data_from_page(html_content):
    ...
    for rank_td in soup.find_all('td', class_='rank-td'):
        ...
        if len(next_siblings) >= 2:
            market_cap = next_siblings[0].get_text(strip=True)
            price = next_siblings[1].get_text(strip=True)
            market_caps_and_prices.append((market_cap, price))
    ...
    current_date = datetime.today().strftime('%Y-%m-%d')  # Use the current date

    return {
        'Rank': ranks,
        'Name': [name for name, code in names_and_codes],
        'Code': [code for name, code in names_and_codes],
        'Market Cap': [market_cap for market_cap, price in market_caps_and_prices],
        'Price': [price for market_cap, price in market_caps_and_prices],
        'Today': today_spans,
        'Country': countries,
...
# Function to scrape and save data for market cap
def scrape_and_save_market_cap_data(initial_url, base_url, num_pages, file_name):
    all_data = []
    ...
    for url in all_urls:
        ...
        if response.status_code == 200:
            page_data = extract_data_from_page(response.content)
    ...
    combined_data = {
        ...
        'Market Cap': [market_cap for data in all_data for market_cap in data['Market Cap']],
    ...
    market_cap_df = pd.DataFrame(combined_data)

    # Append the data to the existing file
    if not os.path.isfile(file_name):
        market_cap_df.to_csv(file_name, index=False)
    else:
        existing_df = pd.read_csv(file_name)
        combined_df = pd.concat([existing_df, market_cap_df])
        combined_df.to_csv(file_name, index=False)

    print(f"Data for Market Cap saved to {file_name}")


# Define URLs and file name for market cap
initial_url = 'https://companiesmarketcap.com/'
base_url = 'https://companiesmarketcap.com/page/{}/'
file_name = 'market_cap_data.csv'


# Function to run the scraping job
def job():
    scrape_and_save_market_cap_data(initial_url, base_url, 86, file_name)
