...
# Define function to extract data from the total assets page
def extract_data_from_total_assets_page(html_content):
    soup = BeautifulSoup(html_content, 'html.parser')

    ranks = [rank.get_text().strip() for rank in soup.find_all('td', class_='rank-td')]

    total_assets_and_prices = []
    for rank_td in soup.find_all('td', class_='rank-td'):
        ...
        if len(next_siblings) >= 2:
            total_assets = next_siblings[0].get_text(strip=True)
            price = next_siblings[1].get_text(strip=True)
            total_assets_and_prices.append((total_assets, price))
    ...
    return {
        ...
        'Total Assets': [total_assets for total_assets, price in total_assets_and_prices],
        'Price': [price for total_assets, price in total_assets_and_prices],
        'Today': today_spans,
        'Country': countries,
...
# Function to scrape and save data for total assets
def scrape_and_save_total_assets_data(initial_url, base_url, num_pages, file_name):
    all_data = []
    ...
    for url in all_urls:
        ...
        if response.status_code == 200:
            page_data = extract_data_from_total_assets_page(response.content)
    ...
    combined_data = {
        ...
        'Total Assets': [total_assets for data in all_data for total_assets in data['Total Assets']],
    ...
    total_assets_df = pd.DataFrame(combined_data)

    # Append the data to the existing file
    if not os.path.isfile(file_name):
        total_assets_df.to_csv(file_name, index=False)
    else:
        existing_df = pd.read_csv(file_name)
        combined_df = pd.concat([existing_df, total_assets_df])
        combined_df.to_csv(file_name, index=False)

    print(f"Data for Total Assets saved to {file_name}")


# Define URLs and file name for total assets
initial_url = 'https://companiesmarketcap.com/top-companies-by-total-assets/'
base_url = 'https://companiesmarketcap.com/top-companies-by-total-assets/page/{}/'
file_name = 'total_assets_data.csv'


# Function to run the scraping job
def job():
    scrape_and_save_total_assets_data(initial_url, base_url, 86, file_name)
