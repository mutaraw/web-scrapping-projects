...
# Define function to extract data from the debt page
def extract_data_from_debt_page(html_content):
    ...
    for rank_td in soup.find_all('td', class_='rank-td'):
        ...
        if len(next_siblings) >= 2:
            debt = next_siblings[0].get_text(strip=True)
            price = next_siblings[1].get_text(strip=True)
            debts_and_prices.append((debt, price))
    ...
    return {
        ...
        'Debt': [debt for debt, price in debts_and_prices],
        'Price': [price for debt, price in debts_and_prices],
        'Today': today_spans,
        'Country': countries,
...
# Function to scrape and save data for debt
def scrape_and_save_debt_data(initial_url, base_url, num_pages, file_name):
    all_data = []
    ...
    for url in all_urls:
        ...
        if response.status_code == 200:
            page_data = extract_data_from_debt_page(response.content)
    ...
    combined_data = {
        ...
        'Debt': [debt for data in all_data for debt in data['Debt']],
    ...
    debt_df = pd.DataFrame(combined_data)

    # Append the data to the existing file
    if not os.path.isfile(file_name):
        debt_df.to_csv(file_name, index=False)
    else:
        existing_df = pd.read_csv(file_name)
        combined_df = pd.concat([existing_df, debt_df])
        combined_df.to_csv(file_name, index=False)

    print(f"Data for Debt saved to {file_name}")


# Define URLs and file name for debt
initial_url = 'https://companiesmarketcap.com/companies-with-the-highest-debt/'
base_url = 'https://companiesmarketcap.com/companies-with-the-highest-debt/page/{}/'
file_name = 'debt_data.csv'


# Function to run the scraping job
def job():
    scrape_and_save_debt_data(initial_url, base_url, 86, file_name)
