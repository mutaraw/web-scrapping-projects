...
# Define function to extract data from the revenue page
def extract_data_from_revenue_page(html_content):
    ...
    for rank_td in soup.find_all('td', class_='rank-td'):
        ...
        if len(next_siblings) >= 2:
            revenue = next_siblings[0].get_text(strip=True)
            price = next_siblings[1].get_text(strip=True)
            revenue_and_prices.append((revenue, price))
    ...
    return {
        ...
        'Revenue': [revenue for revenue, price in revenue_and_prices],
        'Price': [price for revenue, price in revenue_and_prices],
        'Today': today_spans,
        'Country': countries,
...
# Function to scrape and save data for revenue
def scrape_and_save_revenue_data(initial_url, base_url, num_pages, file_name):
    all_data = []
    ...
    for url in all_urls:
        ...
        if response.status_code == 200:
            page_data = extract_data_from_revenue_page(response.content)
    ...
    combined_data = {
        ...
        'Revenue': [revenue for data in all_data for revenue in data['Revenue']],
    ...
    revenue_df = pd.DataFrame(combined_data)

    # Append the data to the existing file
    if not os.path.isfile(file_name):
        revenue_df.to_csv(file_name, index=False)
    else:
        existing_df = pd.read_csv(file_name)
        combined_df = pd.concat([existing_df, revenue_df])
        combined_df.to_csv(file_name, index=False)

    print(f"Data for Revenue saved to {file_name}")


# Define URLs and file name for revenue
initial_url = 'https://companiesmarketcap.com/largest-companies-by-revenue/'
base_url = 'https://companiesmarketcap.com/largest-companies-by-revenue/page/{}/'
file_name = 'revenue_data.csv'


# Function to run the scraping job
def job():
    scrape_and_save_revenue_data(initial_url, base_url, 86, file_name)
